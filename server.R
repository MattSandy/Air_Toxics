...
shinyServer(function(input, output, session) {
    ...
    output$barplot <- renderChart({
        ...
        if (is.null(isolate(input$pollutant)) | is.null(dataset2()) ) {
            ...
            if(length(unique(bar2$SiteId))>1) h2$xAxis(categories=unique(bar2$SiteId), title=list(style=list(fontSize="13px", color="#333333"), margin=8, text="Monitor Site"), type="category", labels=list(rotation=-58, align='right'))
